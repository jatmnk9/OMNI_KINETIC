      ...
      <div className="max-w-lg mx-auto h-screen flex flex-col">
        ...
        <div className="relative h-[45%] w-full bg-neutral-900 overflow-hidden">
            ...
            {filteredFeatures.map((f, i) => (
              ...
              >
                <div className={`relative group`}>
                  <MapPin className={`w-10 h-10 ${selectedLocation?.properties.name === f.properties.name ? 'text-accent scale-125' : 'text-brand opacity-60'}`} />
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 bg-card text-[9px] font-bold uppercase tracking-[0.2em] px-3 py-1.5 rounded-full whitespace-nowrap shadow-2xl border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
                    {f.properties.name}
                  </div>
                </div>
              </div>
          ...
          <div className="absolute bottom-10 left-6 right-6 flex items-center justify-between">
            <Badge className="bg-accent text-white font-bold tracking-[0.2em] uppercase text-[9px] px-3 py-1 rounded-full">3 Points Nearby</Badge>
            <Button size="sm" variant="secondary" className="text-[9px] uppercase tracking-widest font-bold bg-white/10 backdrop-blur-xl rounded-full px-4 h-8">
            ...
            {selectedLocation ? (
              <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="flex gap-4 pt-6">
                  <Button className="flex-1 h-16 bg-brand font-bold uppercase tracking-[0.2em] rounded-2xl shadow-xl">
                    Reserve Refill
                  </Button>
                ...
                <div className="space-y-4">
                  {filteredFeatures.map((f, i) => (
                    <Card 
                      ...
                      onClick={() => setSelectedLocation(f)}
                    >
                      <div className="w-14 h-14 bg-brand/10 rounded-2xl flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
